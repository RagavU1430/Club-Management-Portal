...
export default function About() {
  ...
  return (
    <main className="relative min-h-screen pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      {/* Origin Story Panel */}
      <section className="relative z-10 rounded-3xl glass p-8 sm:p-12 border border-white/10 mb-20 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      ...
      <section className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
        {ACHIEVEMENTS.map((a) => {
          ...
          return (
            <div
              key={a.label}
              className="rounded-2xl glass p-6 border border-white/10 hover:border-cyan-400/40 transition flex flex-col justify-between shadow-sm hover:shadow-lg"
            >
              <div className={`p-3 rounded-xl bg-white/5 border border-white/10 w-fit ${a.color} transition-colors`}>
                <Icon className="h-6 w-6" />
              </div>
              <div className="mt-6">
                <div className="font-display text-3xl font-black text-white">{a.label}</div>
                <p className="mt-2 text-xs text-slate-400 leading-relaxed">{a.desc}</p>
              </div>
            </div>
          );
        })}
      ...
      {activities.length > 0 && (
        <section className="relative z-10 mb-20">
          <div className="text-center max-w-xl mx-auto mb-12">
            <span className="text-xs font-mono text-cyan-400 uppercase tracking-widest flex items-center justify-center gap-1.5 mb-2 font-semibold">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {activities.map((act) => (
              <div
                key={act.id}
                className="group glass rounded-2xl overflow-hidden border border-white/10 hover:border-cyan-400/40 transition-all flex flex-col justify-between shadow-sm hover:shadow-xl"
              >
                <div className="relative h-48 w-full bg-slate-900/60 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="rounded-full bg-cyan-400 text-black px-2.5 py-0.5 text-[10px] font-mono font-bold uppercase tracking-wider shadow">
      ...
      <section className="relative z-10 mb-20">
        <div className="text-center max-w-xl mx-auto mb-12">
          <span className="text-xs font-mono text-purple-400 uppercase tracking-widest font-semibold">CHRONOLOGY</span>
          <h2 className="mt-2 font-display text-3xl font-bold text-white">Evolution of the Guild</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {TIMELINE.map((item, idx) => (
            <div
              key={item.year}
              className="relative rounded-2xl glass p-6 border border-white/10 hover:border-purple-400/40 transition shadow-sm hover:shadow-lg"
            >
              <div className="flex items-center justify-between mb-4">
                <span className="font-display text-2xl font-black text-purple-400">{item.year}</span>
      ...
      <section className="relative z-10 mb-20">
        <div className="text-center max-w-xl mx-auto mb-12">
          <span className="text-xs font-mono text-cyan-400 uppercase tracking-widest font-semibold">COMMUNITY VOICES</span>
          <h2 className="mt-2 font-display text-3xl font-bold text-white">Alumni & Member Impact</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {TESTIMONIALS.map((t) => (
            <div key={t.name} className="rounded-2xl glass p-6 border border-white/10 flex flex-col justify-between shadow-sm hover:shadow-md transition">
              ...
              <div className="mt-6 pt-4 border-t border-white/5">
                ...
                <div className="text-xs text-cyan-300 font-mono mt-0.5">{t.role}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {FAQS.map((faq, idx) => {
            ...
            return (
              <div
                key={faq.q}
                className="rounded-xl glass border border-white/10 overflow-hidden transition shadow-sm"
              >
                <button
                  onClick={() => setOpenFaq(isOpen ? null : idx)}
                  className="w-full flex items-center justify-between p-5 text-left font-medium text-white hover:text-cyan-300 transition cursor-pointer"
                >
      ...
      {/* Bottom CTA */}
      <section className="relative z-10 rounded-3xl overflow-hidden border border-cyan-400/30 bg-gradient-to-r from-cyan-950/40 via-[#070b16] to-purple-950/40 p-10 text-center shadow-xl backdrop-blur-md">
        <h2 className="font-display text-2xl sm:text-4xl font-black text-white glow-text">
          Join the Movement
        </h2>
