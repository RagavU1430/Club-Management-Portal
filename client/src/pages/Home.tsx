    ...
    <main className="relative w-full">
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex flex-col items-center text-center mb-14">
          <h2 className="font-display text-3xl sm:text-5xl font-black text-slate-900 dark:text-white glow-text transition-colors">
            What We Learn, Code & Build
          </h2>
          <p className="mt-4 text-base text-slate-600 dark:text-slate-400 max-w-2xl transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {TRACKS.map((track) => {
            ...
            return (
              <div
                key={track.title}
                className={`group relative rounded-2xl glass p-6 transition-all duration-300 hover:-translate-y-1.5 border border-slate-200/80 dark:border-white/10 ${track.border} overflow-hidden shadow-sm hover:shadow-xl`}
              >
                ...
                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div>
                    <div className="h-12 w-12 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center text-cyan-600 dark:text-cyan-300 group-hover:scale-110 group-hover:border-cyan-500/40 dark:group-hover:border-cyan-400/40 transition-transform duration-300">
                      <Icon className="h-6 w-6" />
                    </div>

                    <h3 className="mt-5 font-display text-xl font-bold text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-300 transition-colors">
                      {track.title}
                    </h3>
                    <p className="mt-3 text-sm text-slate-600 dark:text-slate-300 leading-relaxed transition-colors">
                      {track.desc}
                    </p>
                  </div>

                  <div className="mt-6 flex flex-wrap gap-1.5 pt-4 border-t border-slate-200 dark:border-white/5">
                    {track.tags.map((t) => (
                      <span
                        key={t}
                        className="rounded-md bg-slate-100 dark:bg-white/5 px-2 py-0.5 text-[11px] font-mono text-slate-600 dark:text-slate-400 group-hover:text-cyan-700 dark:group-hover:text-slate-200 transition"
                      >
                        #{t}
                      </span>
                    ))}
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
        <div className="relative rounded-3xl overflow-hidden border border-cyan-500/30 dark:border-cyan-400/30 bg-gradient-to-r from-cyan-500/10 via-white/80 to-purple-500/10 dark:from-cyan-950/50 dark:via-[#070b16] dark:to-purple-950/50 p-8 sm:p-14 text-center shadow-xl dark:shadow-[0_0_80px_rgba(0,240,255,0.15)] backdrop-blur-md">
          {/* Ambient Lighting */}
          ...
          <div className="relative z-10 max-w-2xl mx-auto">
            <span className="inline-block text-xs font-mono text-cyan-700 dark:text-cyan-300 tracking-widest uppercase mb-3 font-semibold">
              ZERO ENTRY BARRIER // UNRESTRICTED INNOVATION
            </span>
            <h2 className="font-display text-3xl sm:text-5xl font-black text-slate-900 dark:text-white glow-text transition-colors">
              Ready to Build the Future of AI?
            </h2>
            <p className="mt-4 text-sm sm:text-base text-slate-600 dark:text-slate-300 transition-colors">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link
                to="/team"
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl glass px-8 py-3.5 font-bold text-slate-900 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition shadow-sm"
              >
                Meet the Leadership
              </Link>
            </div>
